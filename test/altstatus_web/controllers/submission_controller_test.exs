defmodule AltstatusWeb.SubmissionControllerTest do
  use AltstatusWeb.ConnCase

  alias Altstatus.Resources
  alias Altstatus.Resources.Submission

  @create_attrs %{medium_url: "some medium_url", reflection: "some reflection", twitter_url: "some twitter_url"}
  ...
  def fixture(:submission) do
    {:ok, submission} = Resources.create_submission(@create_attrs)
    submission
  end
  ...
  describe "update submission" do
    ...
    test "renders submission when data is valid", %{conn: conn, submission: %Submission{id: id} = submission} do
      ...
      assert json_response(conn, 200)["data"] == %{
        "id" => id,
        "medium_url" => "some updated medium_url",
        "reflection" => "some updated reflection",
